...
public final class Day2Year2019: DaySolverWithInputs {
    ...
    public func solvePart1() -> String {
        Self.executedProgram(restoredProgram)[0]
            .description
    }
    ...
    public func solvePart2() -> String {
        let inputCombinations = product(0 ... 99, 0 ... 99)
        let matchingInput = inputCombinations
            .lazy
            .map({ (noun: Int, verb: Int) -> (noun: Int, verb: Int, output: Int) in
                let program = self.modifiedProgram(noun: noun, verb: verb)
                let output = Self.executedProgram(program)[0]
                return (noun: noun, verb: verb, output: output)
            })
            .first(where: { $0.output == 19690720 })!
        let result = 100 * matchingInput.noun + matchingInput.verb
...
public extension Day2Year2019 {
    typealias InputElement = Int

    typealias Program = [InputElement]
...
extension Day2Year2019.OpCode: ParseableFromString {}

// MARK: - Logic

extension Day2Year2019 {
    public static func executeProgram(_ program: inout Program) {
        var index = 0
        loop: while true {
            guard let opcode = OpCode(rawValue: program[index]) else {
                fatalError("Invalid opcode: \(program[index])")
            }
    ...
    public static func executedProgram(_ program: Program) -> Program {
        updated(program, with: { executeProgram(&$0)} )
    }

    private func modifiedProgram(noun: Int, verb: Int) -> Program {
        updated(inputElements, with: {
            $0[1] = noun
            $0[2] = verb
        })
    }

    private var restoredProgram: Program {
        modifiedProgram(noun: 12, verb: 02)
    }
}
