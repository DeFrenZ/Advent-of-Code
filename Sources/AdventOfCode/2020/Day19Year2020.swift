...
public final class Day19Year2020: DaySolverWithSingleInput {
    ...
    public func solvePart1() -> String {
        let rulesByID = self.rulesByID
        let rule0 = rulesByID[0]!
        return input.messages
            .count(where: { Self.fullMatch($0, to: rule0, in: rulesByID) })
            .description
    }
    ...
    public func solvePart2() -> String {
        let rulesByID = updated(self.rulesByID) {
            $0[8]!.pattern = .subRules([[42], [42, 8]])
            $0[11]!.pattern = .subRules([[42, 31], [42, 11, 31]])
        }
        let rule0 = rulesByID[0]!
        return input.messages
            .count(where: { Self.fullMatch($0, to: rule0, in: rulesByID) })
...
public extension Day19Year2020 {
    struct Input {
    ...
    struct Rule: Identifiable {
        ...
        public enum Pattern {
            ...
            case subRules([[Rule.ID]])
        }
    }
}

extension Day19Year2020.Input: ParseableFromString {
...
extension Day19Year2020.Rule.Pattern: ParseableFromString {
    var description: String {
        switch self {
        case .character(let character):
            return #""\#(character)""#
        case .subRules(let subRules):
            return subRules
...
extension Day19Year2020 {
    var rulesByID: [Rule.ID: Rule] {
        .init(indexingValuesByID: input.rules)
    }

    static func fullMatch(
        _ string: String,
        to rule: Rule,
        in rulesByID: [Rule.ID: Rule])
    -> Bool {
        match(string, to: rule, in: rulesByID)
            .contains(where: { $0 == string.endIndex })
    }

    static func match <S: StringProtocol> (
        _ string: S,
        to rule: Rule,
        in rulesByID: [Rule.ID: Rule])
    -> AnySequence<S.Index> {
        switch rule.pattern {
        case .character(let character):
            guard string.first == character else { return [].eraseToAnySequence() }
            return [string.index(after: string.startIndex)]
                .eraseToAnySequence()
        case .subRules(let subRulesOptions):
            return subRulesOptions
                .lazy
                .flatMap({ subRulesIDs in
                    subRulesIDs.reduce(into: [string.startIndex], { indices, subRuleIDs in
                        let subRule = rulesByID[subRuleIDs]!
                        indices = indices.flatMap({ index in
                            match(string[index...], to: subRule, in: rulesByID)
                        })
                    })
                })
                .eraseToAnySequence()
