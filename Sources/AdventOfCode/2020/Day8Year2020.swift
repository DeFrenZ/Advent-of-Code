...
public final class Day8Year2020: DaySolverWithInputs {
    ...
    public func solvePart2() -> String {
        inputElements
            .tentativeReparations()
            .map({ $0.execute() })
            .first(where: { $0.stopReason == .termination })!
            .accumulator
            .description
    }
}
...
extension Day8Year2020.Instruction: ParseableFromString {
    public var description: String {
        let argumentDescription = String(format: "%+d", argument)
    ...
    public static func parse(on scanner: Scanner) throws -> Self {
        ...
        let argument = try scanner.scanInt() ?! ParseError.onArgument(scanner.remainingString)
    ...
    enum ParseError: Error {
        ...
        case onSpace(String)
        case onArgument(String)
    }
}

extension Day8Year2020.Instruction.Operation: ParseableFromString {}

// MARK: - Logic

extension Day8Year2020.Program {
    func execute() -> (stopReason: ExecutionStopReason, accumulator: Int) {
        var visitedInstructions: Set<Int> = []
        var nextIndex: Int = 0
        var accumulator: Int = 0
        while true {
            if visitedInstructions.contains(nextIndex) {
                return (.loop, accumulator)
            }
            if nextIndex == endIndex {
                return (.termination, accumulator)
            }
            visitedInstructions.insert(nextIndex)
            (nextIndex, accumulator) = executeInstruction(atIndex: nextIndex, accumulator: accumulator)
        }
    }
    ...
    func executeInstruction(atIndex index: Int, accumulator: Int) -> (nextIndex: Int, accumulator: Int) {
        ...
        case .jump:
            return (
                nextIndex: index + instruction.argument,
    ...
    func tentativeReparations() -> AnySequence<Self> {
        indices
            .lazy
            .compactMap({ index in
                switch self[index].operation {
                case .accumulator:
                    return nil
                case .jump:
                    return updated(self) { $0[index].operation = .noOperation }
                case .noOperation:
